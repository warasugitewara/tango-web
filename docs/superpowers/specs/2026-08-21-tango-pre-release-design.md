...
- デッキ一覧APIと画面は総カード数と1日あたりの新規上限を返す。当日の実残数は学習セッション開始後に算出するため、デッキ一覧では表示しない。
- デッキ更新・論理削除APIは実装したが、プレリリース画面には編集・削除操作を置かない。
- カード一覧APIは `limit` / `offset` でページングする。プレリリース画面は先頭50件だけを表示し、ページ送りUIは後続とする。
- `backup.sh` はJSTの日付ごとに1件だけdumpを作る。cronまたはsystemd timerそのものはLXC固有設定としてリポジトリに含めず、配置手順に設定責任を明記する。
- 本番のPostgreSQL接続URLは、秘密値を環境変数へ直接置かないため `DATABASE_URL_FILE` から読む。開発・テスト用の `DATABASE_URL` は互換経路として残す。

上記の未実装UIと運用自動化は `docs/todo/pre-release-deferred.md` に影響と後続フェーズを記録する。復習取引、所有者境界、04:00 JSTの学習日、取り込み、SPA同一オリジンなど、受け入れ条件に必要な契約は変更しない。
